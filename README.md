...
A full-featured **secure P2P chat app** built with Java and JavaFX. It supports group chat, private chat, and file transfer, with a robust security architecture and a truly distributed network design.
...
```cmd
# Smart start (auto-selects GUI or CLI)
start.bat
...
> **Note:** If you see a console instead of the GUI, your Java environment is missing JavaFX. See the JavaFX troubleshooting section below.
...
### Issue 1: Console appears instead of GUI

**Symptom:** After running `start.bat`, a black console window appears instead of the graphical interface.

**Cause:** JavaFX is missing from your Java environment, so the script falls back to the CLI.

**Fix:** See “Issue 2: Missing JavaFX runtime components” below.

### Issue 2: Missing JavaFX runtime components

**Error:** `Error: JavaFX runtime components are missing` or `Module javafx.controls not found`
...
1. **Easiest (recommended):**
   ...
   * Re-run `start.bat` after installation
...
3. **Manual JavaFX install:**

   * Download the JavaFX SDK: [https://openjfx.io/](https://openjfx.io/)
   * Extract it (e.g., to `C:\javafx-sdk-17.0.2`)
   * Run:
...
├── src/                          # Source code
├── target/                       # Build output
├── start.bat / start.sh          # Main launch scripts
├── scripts/                      # Additional launchers
...
* **Secure file transfer:** Files sent over encrypted channels
...
2. Try a different launcher script
